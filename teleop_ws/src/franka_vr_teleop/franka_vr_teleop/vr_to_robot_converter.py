...
class VRToRobotConverter(Node):
    def __init__(self):
        ...
        self.epsilon_inner = 0.005  # m
        self.epsilon_outer = 0.005  # m
        self.gripper_state  = np.array([self.index_button_pressed, self.gripper_speed, self.gripper_force, self.epsilon_inner, self.epsilon_outer]) # position m, speed m/s, force N
    ...
    def receive_vr_data(self):
        ...
        while rclpy.ok():
            try:
                ...
                # If we received exactly 72 bytes, treat it as binary pose (9 doubles)
                if len(data) == 72:
    ...
    def parse_vr_binary(self, data, addr=None):
        """Parse 72-byte binary pose: 9 doubles (x,y,z, qx,qy,qz,qw, press_index, pause_val) little-endian"""
        try:
            if len(data) != 72:
                self.get_logger().warn(f"Binary pose of unexpected length {len(data)}")
                return

            # Unpack 9 little-endian doubles
    ...
    def send_robot_command(self, position, orientation, gripper_state):
        ...
        try:
            # Send absolute pose: position (x,y,z) and orientation (qx,qy,qz,qw) and gripper state (button_pressed, speed, force, epsilon_inner, epsilon_outer)
            message = f"{position[0]:.6f} {position[1]:.6f} {position[2]:.6f} " + \
                        f"{orientation[0]:.6f} {orientation[1]:.6f} {orientation[2]:.6f} {orientation[3]:.6f} " + \
                        f"{gripper_state[0]:.6f} {gripper_state[1]:.6f} {gripper_state[2]:.6f} {gripper_state[3]:.6f} {gripper_state[4]:.6f}"
